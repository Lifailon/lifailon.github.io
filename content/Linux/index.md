...
Системные вызовы (`Syscall`) - это единственный способ для программы в `User Space` попросить ядро выполнить действие, требующее повышенных привилегий (например, прочитать или записать в файл, отправить пакет в сеть или создать новый процесс).

- `fork()`/`clone()` - создать копию текущего процесса (дочерний процесс) или потоки.
...
- `poll()`/`epoll_wait()` - ждать получение данных (например, из открытого сетевого соединения с БД).
- `exit()` - завершить работу процесса.
- `open()`/`openat()` - открыть файл.
- `read()` и `write()` - чтение и запись.
- `stat()`/`newfstatat()` - получить информацию о файле (размер, права).
...
`strace` (system call trace) - это инструмент трассировки системных вызово при работе процессов в Linux, которое выводит каждое обращение программы к ядру Linux, например, для чтение файла или отправки пакета в сеть.

- `strace -c uptime > /dev/null` вывести статистику (`-c`) всех используемых системных вызовов при запуске программы (`% time` - процент от общего времени выполнения)
- `strace -T -e trace=file uptime > /dev/null` - вывести все системные вызовы, относящиеся к файловой системе
- `strace -T -e trace=openat uptime > /dev/null` - фильтрация по системному вызову
- `strace -T -e trace=network host google.com > /dev/null` - вывести все сетевые вызовы
- `strace -T -e trace=connect host google.com > /dev/null` - системные вызовы на создание соединения
- `strace -f -o debug.log ./app` - запустить приложение, которое падает с ошибкой с записью в лог (`-f` включая дочерние процессы)
- `strace -Z -p <PID>` - вывести только неудачные вызовы (где возвращяется код ошибки)
...
Ядро не выбирает процесс случайно. Оно начисляет каждому процессу очки (`badness score`) от 0 до 1000, чем значение больше, тем выше шанс, что процесс будет убит. Значение можно увидеть с помощью cat `/proc/[PID]/oom_score`. Если в файле процесса `/proc/[PID]/oom_score_adj` записать `-1000`, `OOM Killer` будет игнорировать этот процесс. В логе ядра (`dmesg | grep -i oom`) отображается какой процесс убит, сколько памяти он занимал и какой у него был `oom_score`.
...
`LA` (Load Average) - среднее (`avg`) количество процессов в состоянии `R` и `D` за 1, 5 и 15 минут.

Когда высокий `Load Average`, это значит процессов больше, чем ядер для их выполнения (тем самым образуется очередь). Частый кейс, если приложение начало отвечать пользователям по 10 секунд, при этом процессор не загружен и доступно много свободной памяти.

В показатель `LA` попадают процессы в состояние Running (`R`), которые реально используют CPU прямо сейчас и Uninterruptible Sleep (`D`), которые ожидают своей очереди на выполнение (например, ответа от базы данных, внешнего API или медленного диска). Когда программа запрашивает данные с диска, она передает управление операционной системе и игнорирует любые сигналы, включая `SIGKILL` (`kill -9`). Процессы в режиме прерываемого сна - Interruptible Sleep (`S`), не попадают в `LA`.

- Анализировать ввод/вывод (I/O) диска, например, с помощью `top` или `vmstat`, искать высокий показатель `wa` или `%iowait` в `iostat`, возможно, диск перегружен записью логов или большое количество транзакций в локальную БД (необходимо искать открытые файлы с помощью `lsof`).
- Анализировать ввод/вывод сети, например, с помощью `ss`, где может быть больше количество соединений в статусе `TIME_WAIT` или `CLOSE_WAIT`. Когда приложение хочет сделать запрос к базе, оно открывает TCP-соединение, если соединений слишком много, новые запросы встают в очередь на уровне операционной системы и приложение просто ждет свободный порт или свободный слот в пуле.
- Анализировать системные вызовы, например, приложение могло зациклиться на попытке прочитать несуществующий файл конфигурации, с помощью системного вызова `read()`.

### Процессы

Поиск PID процесса по его названию:

```bash
proc="ping"
for p in /proc/[0-9]*; do 
  if grep -q "$proc" "$p/comm" 2>/dev/null; then 
    echo "${p##*/}"
  fi
done
```

Узнать статус работы процесса: `cat /proc/<PID>/status | grep State`

Анализ работы процесса для переноса приложения с одного хоста на другой, где известен только порт, на котором запущено приложение.

- Узнаем кто слушаем порт (имя процесса и PID): `sudo lsof -i :8080` и на каком адресе `ss -lntp | grep 8080` (может быть привязан к текущему IP-адресу или `localhost`)
- Путь к исполняемому файлу: `ls -l /proc/[PID]/exe`
- Рабочая директория (откуда запущен процесс): `ls -l /proc/[PID]/cwd`
- Аргументы/флаги при запуске приложения: `cat /proc/[PID]/cmdline` (например, путь к файлу `.jar` или конфигурации)
- Переменные окружения, которые доступны процессу: `strings /proc/[PID]/environ` или `cat /proc/[PID]/environ | tr '\0' '\n'`
- Открытые файлы процессом (например, лог-файлы, конфигурации или локальные базы данных): `sudo lsof -p [PID] | grep -E '\.(log|conf|yml|yaml|json|db|sqlite)'` или `ls -l /proc/[PID]/fd` (для каждого дискриптора своя символическая ссылка)
- Найти файл, который удерживает 1-й (`stdout`) дескриптор вывода (например, при запуске в `nohup` или перенаправление вывода в файл): `ls -l /proc/[PID]/fd/1`
- Используемые бинарники и библиотеки (`.so`) процессом (для определения ЯП и зависимостей): `sudo cat /proc/[PID]/maps | awk '{print $2,$6}' | grep '/' | sort -u`
- Показывает список динамических библиотек (List Dynamic Dependencies), которые требуются бинарному файлу для запуска (например, `.so` - Shared Object, использумые в `C`/`C++`/`Java`/`C#`): `ldd [путь_к_exe]`
- Отобразить системные вызовы записи на уровне ядра: `sudo strace -p [PID] -s 1000 -e write # -e write=1,2`

Проверить возможные варианты запуска:

- Найти возможный путь к файлу юнита systemd: `systemctl status [process_name]`, `systemctl cat [process_name]`, `systemctl list-units --type=service | grep [process_name]` или пользовательские `systemctl --user list-units`.
- Узнать все существующие пути к юнит файлам в системе: `systemd-analyze unit-paths` и ищем юниты вручную: `ls /etc/systemd/system/` (системные) или `/etc/systemd/user/` (пользовательские от администратора), `/lib/systemd/system/` (пакеты), `~/.config/systemd/user/` (пользовательские), `/run/systemd/system/` (временные/runtime юниты).
- Запуск с помощью планировщика: `crontab -l` или `ls /etc/cron.d/`
- Запуск в контейнере: `docker ps | grep 8080`
- Ручной запуск с помощью `tmux` или `nohup` (игнорирует `SIGHUP` при закрытие терминала и переводит потоки вывода в файл `nohup.out`): `ps aux | grep [process_name/PID]` (если родитель процесса `PPID` равен 1, значит оно отвязано от терминала).

### Статус процессов

Статус работы процессов в `top` или `ps aux`

- `Running` (*R*) - исполняется или ждет в очереди на выполнение.
- `Sleeping` (*S*/*D*) - `S` прерываемый (ожидает событие ввода пользователя или сетевой пакет), `D` непрерываемый (чаще ждет `I/O` от диска, нельзя убить с помощью `kill -9`).
...
Статус нагрузки на систему в `top -bn1 | grep %Cpu` (средний показатель с момента загрузки системы) или текущий `top -bn2 -d 0.5 | grep %Cpu | tail -n 1`

`%Cpu(s):  0.9 us,  0.7 sy,  0.0 ni,  98.4 id,  0.0 wa,  0.0 hi,  0.0 si,  0.0 st`

- `us` (*user*) - время на работу пользовательских приложений, внутри его собственной памяти (например, обработка цикла, сортировка массива и т.п.).
- `sy` (*system*) - время на обработку системных вызовов ядром (например, приложение отправляет пакет в сеть или записывает строку в лог-файл).
...
- `ss -t` - отобразить только установленные TCP-соединения в статусе `ESTAB`.
- `ss -tl` - отобразить только те сокеты, которые ждут входящих соединений в статусе `LISTEN`.
- `ss -ta` - отобразить все TCP-сокеты (в состояние `LISTEN`, `ESTAB` и другие).
- `ss -ua` - отобразить все UDP-сокеты и их порты, которые открыты в системе (чаще в состоянии `UNCONN`, т.к. сокет только слушают порт, либо быстро пересылает данные или `ESTAB` если сокет привязн к конкретному IP и порту удаленной стороны, например, в VPN-туннелях или потоковом видео).
...
`set -x` - включает режим xtrace, при котором печатается каждая команда перед выполнением (подставляя значения переменных, как в `bash -x`) \
...
trap 'echo "Ошибка на $LINENO строке при выполнении команды: $BASH_COMMAND"' ERR
...
Терминал вернет текст `Ошибка на 12 строке при выполнении команды: false` и завершится.
...
# пути
which kubectl # узнать путь до используемого исполняемого файла
which -a kubectl # список всех исполняемых файлов в системе
hash -r # очистить кеш путей при удаление первого исполняемого файла в списке
basename $(which kubectl) # kubectl
dirname $(which kubectl) # /usr/bin
...
# Интерактивный поиск по истории команд (заменяет встроенный Ctrl+R)
...
fdfind -t f 2> /dev/null | fzf --preview "batcat --color=always {}"
# Поиск всех файлов с расширением yml/yaml в системе
sudo find / -type f "(" -name "*.yml" -o -name "*.yaml" ")" | fzf
sudo find / -type f -regex ".+\.ya?ml$" | fzf
sudo fdfind -HI -t f '\.ya?ml$' / | fzf
...
```bash
# Шаг. Запускать каждые 15 минут
*/15 * * * *
# Список. Запускать каждые 15 минут
0,14,29,44 * * * *
# Диапазон. Запускать каждый час с 9 до 18 по будням
0 9-18 * * 1-5
# В 12:00 10-го числа каждого месяца в любой день недели
0 12 10 * *
```
...
`lsof` (list open files) - утилита для доступа к таблице файловых дескрипторов, которая выводит список всех открытых в данный момент файлов и процессов, которые их используют (файловые дискрипторы), включая сетевые сокеты (открытые порты и соединения), `FIFO` (`pipe`), символьные и блочные устройства (терминалы, диски, `/dev/null`).
...
`FD` (File Descriptor) - отображает, как именно файл используется

- `cwd` - текущий рабочий каталог процесса
- `rtd` (Root Directory) - корневой каталог процесса, если процесс запущен в изолированном окружении (например, через `chroot`)
- `txt` - исполняемый файл (текст программы)
- `mem` - файл в оперативной памяти (memory-mapped file), например, общие библиотеки
- Номер дескриптора (например, `0u` и `1w`) и режим доступа - `r`/`w`/`u` (чтение, запись или чтение и запись)

`TYPE` - тип файла, связанного с дескриптором

- `REG` - обычный файл
- `DIR` - каталог
- `CHR` - специальный символьный файл устройства (например, терминалы `/dev/pts/*` или `/dev/null`)
- `BLK` - блочное устройство (например, жесткий диск)
- `FIFO` - канал (`pipe`) для прямого обмена данными между процессами
- `sock` - сетевой (`TCP`) или `UNIX` сокет
- `netlink` - сокет для общения между ядром и процессами в пользовательском пространстве (передается служебная информация о состоянии системы).
- `a_inode` - анонимные индексные дескрипторы (используются для объектов, у которых нет пути в файловой системе, но есть дескриптор, чаще механизмы межпроцессного взаимодействия или уведомлений ядра)
...
[ufw-docker](https://github.com/chaifeng/ufw-docker) - это инструмент для исправления уязвимости, при которой Docker открывает порты контейнеров в обход правил `UFW`. Решение модифицирует iptables, заставляя Docker-контейнеры соблюдать правила UFW через команду `ufw-docker`.
...
ufw-docker allow prowlarr 9696/tcp
ufw-docker allow 3x 443/tcp
ufw-docker allow 3x 2053/tcp
ufw-docker allow telemt 4343/tcp
ufw-docker allow wg 51820/udp
ufw-docker allow wg 51821/tcp
ufw-docker status
```
